...
void loop()
{
  ...
  //variables to hold minimum and maximum acceleration
  float min_accel;
  float max_accel;

  //initialize the minimum and maximum values
  min_accel=max_accel=sensor_readings[0];
  ...
  for (int i=0;i<num_samples;i++)
  {
    ...
    // see if the current value is above our maximum
    if (acceleration > max_accel)
    {
      max_accel = acceleration;
    }
  
    // see if the current value is below our minimum
